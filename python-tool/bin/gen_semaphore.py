...
class gen_semaphore(CfgBase):
    """
        Define class gen_semaphore with attribute(s) and method(s).
    ...
    VERBOSE = 'gen_semaphore'
    __CONFIG = '/../conf/gen_semaphore.cfg'
    ...
    def __init__(self, verbose=False):
        """
            ...
            :exceptions: None
        """
        verbose_message(gen_semaphore.VERBOSE, verbose, 'Initial configuration')
        current_dir = Path(__file__).resolve().parent
        base_config_file = "{0}{1}".format(current_dir, gen_semaphore.__CONFIG)
        CfgBase.__init__(self, base_config_file, verbose=verbose)
        if self.tool_status:
            self.add_new_option(
                gen_semaphore.__OPS[0],
                gen_semaphore.__OPS[1],
                dest='option',
                help='Option tool'
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_status:
            ...
            if num_of_args_sys > 1:
                option = sys.argv[1]
                if option not in gen_semaphore.__OPS:
                    sys.argv = []
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            error_message('gen_semaphore', 'Tool is not operational')
